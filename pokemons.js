...
function kartachqar(kartalar) {
	let boxFather = document.querySelector(".all-card")
	boxFather.innerHTML = "";
	let newcard = kartalar.map((element) => {
        ...
        <div class="card">
            ...
            <p class="qanaqaligi">${element.type}</p>
            <p class="obrosi">Candy Count: ${element.candy_count}</p>
            <p class="ogirligi">${element.weight}</p>
            <p class="turi">${element.weaknesses}</p>

            <div class="vaqti">
                <p class="${element.spawn_time}</p>
            </div>
        </div>
        `
...
input.addEventListener("input", () => {
	function qidiruv(qidir) {
		...
		let topdi = qidir.filter((malumot) => malumot.name.toLowerCase().includes(inputQiymati))
		kartachqar(topdi)
	}
...
btn.addEventListener("click", () => {
	function azas() {
		...
		if (select === "A-Z") {
			let tartibla = pokemons.sort((a, b) => a.name.localeCompare(b.name))
			kartachqar(tartibla)
		} else if (select === "Z-A") {
			let tartibla = pokemons.sort((a, b) => b.name.localeCompare(a.name))
			kartachqar(tartibla)
		} else {
			kartachqar(pokemons)
		}
...
btn.addEventListener("click", () => {
	function azas() {
		...
		if (select === "0-300kg") {
			let tartibla = pokemons.sort((a, b) => a.weight.localeCompare(b.weight))
			kartachqar(tartibla)
		} else if (select === "300-0kg") {
			let tartibla = pokemons.sort((a, b) => b.weight.localeCompare(a.weight))
			kartachqar(tartibla)
		} else {
			kartachqar(kartalar)
		}
